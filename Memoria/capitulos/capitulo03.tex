...
\begin{lema} \label{lemma:Schur}
  Sea $K:L^2(\mathbb{R}^d) \rightarrow L^2(\mathbb{R}^d)$ un operador tal que $\forall f \in L^2(\mathbb{R}^d)$, $Kf(x)=\int f(u)k(x,u)du$ verificando
  ...
  \noindent Entonces $\|K\| \leq C$. Donde $||K||$ es la norma en $L^2(\mathbb{R}^d)$ de $K$.
\end{lema}

\medskip

\noindent El lema anterior es conocido como Lema de Schur. Sin embargo, como se trata de un lema auxiliar que emplearemos para la demostración del teorema principal de la sección, no lo demostraremos.
