...
\noindent Sin embargo, la invaianza por traslaciones no es suficiente, necesitamos también que nuestro operador sea invariante frente a pequeñas deformaciones (difeomorfismos). De esta forma, un operador $\Phi(f)$ diremos que es estable frente a deformaciones si verifica \autoref{def::Lipschitz_cont}.
...
\begin{observacion}\label{lemma:TF_inestable_difeomorfismos}
  ...
  \begin{align*}
    ...
    \left|\int_{\mathbb{R}} \left(  \frac{1}{1+(\xi - \omega)^2} - \frac{\frac{1}{(1-\epsilon)}}{1+\left[ \frac{(1-\epsilon) \xi - \omega}{(1-\epsilon)}\right]^2}  \right)^2 d\omega \right| \\
    &=\left|\int_{\mathbb{R}} \left(  \frac{1}{1+t^2} - \frac{\frac{1}{(1-\epsilon)}}{1+\left[ \frac{t+\epsilon\xi}{(1-\epsilon)}\right]^2}  \right)^2 dt \right| \\
  \end{align*}
...
\begin{figure} [!h]
  ...
  \caption{Como podemos ver, en el caso de la izquierda la ondleta (en color azul) se ve afectada por una menor escala, lo que le permitirá detectar con mayores frecuencias los cambios que se producen en la señal al convolucionar con esta a lo largo del tiempo. En cambio, en el segundo caso, se ve afectada por una escala mayo, lo que le impedirá detectar con tanta precusion los cambios que se produzcan en la señal. Imagen extraida de \cite{WAVELETS}.}
...
\noindent Antes de comenzar la sección vamos a aclarar la notación que usaremos de ahora en adelante: 
...
\noindent Dado que se tratan de dos funciones positivas las que se restan en el integrnado, la única forma de que el resultado de esta integral de cero es que ambas funciones coincidan punto a punto.
...
En el caso particular de $p+\lambda=(\lambda_1,\lambda_2,\ldots,\lambda_m,\lambda)$
\end{definicion}
...
\noindent Teniendo en cuenta la \autoref{proposicionSumaCaminos} se tiene que  $U[\lambda]U[p]=U[p+\lambda]$, y $A_JU[p]=S_J[p]$, dando lugar a: 
...
\begin{proof}
  ...
  \noindent Dónde se ha usado el cambio de variabel $u'=x-u$ con $Jacobiano=1$.
\end{proof}
