...
\noindent Para obtener mejores propiedades vamos a imponer también que $||M[\lambda]h||=||h|| \; \; \forall h \in L^2(\mathbb{R}^d)$, lo que implica entonces que $|M[\lambda]h|=|h|$. 
...
\noindent Si recordamos la \autoref{proposicionSumaCaminos}, $U[\lambda]U[p]=U[p+\lambda]$, y $A_JU[p]=S_J[p]$, esto da lugar a : 
